...
import org.lucoenergia.conluz.infrastructure.admin.supply.sharingagreement.SharingAgreementResponse;
...
public class CreateSharingAgreementController {
    ...
    @PostMapping
    @Operation(
            summary = "This endpoint creates a new sharing agreement with the specified start and end dates.",
            description = """
    ...
    public SharingAgreementResponse createSharingAgreement(@Valid @RequestBody CreateSharingAgreementBody body) {
        CreateSharingAgreement command = new CreateSharingAgreement(body.getStartDate(), body.getEndDate(), body.getNotes());
        SharingAgreement sharingAgreement = service.create(command);
