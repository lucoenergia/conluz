package org.lucoenergia.conluz.infrastructure.production.sharingagreement.reopen;
...
@Transactional
class ReopenCoefficientsControllerTest extends BaseControllerTest {
    ...
    private String url(UUID plantId, UUID agreementId) {
        return "/api/v1/plants/" + plantId + "/sharing-agreements/" + agreementId + "/partition-coefficients/reopen";
    }

    private String body(UUID... coefficientIds) {
        String ids = String.join(",", java.util.Arrays.stream(coefficientIds).map(id -> "\"" + id + "\"").toList());
        return "{\"coefficientIds\": [" + ids + "]}";
    }
    ...
    @Test
    void returnsConflictWhenCascadeWrittenCloseCannotBeReopened() throws Exception {
        setUpBaseFixture();
        SharingAgreementEntity agreementN = persistAgreement(plantA, SharingAgreementStatus.PUBLISHED);
        Instant boundary = Instant.parse("2024-06-01T00:00:00Z");
        SupplyPartitionCoefficientEntity closedByCascade = persistCoefficient(supplyA, plantA, agreementN,
                Instant.parse("2024-01-01T00:00:00Z"), boundary);
        SharingAgreementEntity agreementNPlus1 = persistAgreement(plantA, SharingAgreementStatus.PUBLISHED);
        persistCoefficient(supplyA, plantA, agreementNPlus1, boundary, null); // starts exactly at boundary
        String authHeader = loginAsCommunityAdmin(communityA.getId());

        mockMvc.perform(post(url(plantA.getId(), agreementN.getId()))
                        .header(HttpHeaders.AUTHORIZATION, authHeader)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body(closedByCascade.getId())))
                .andDo(print())
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.errors[0].code").value("SHARING_AGREEMENT_COEFFICIENT_HAS_SUCCESSOR"));
    }
    ...
    void returnsBadRequestWhenCoefficientIdsEmpty() throws Exception {
        ...
        mockMvc.perform(post(url(plantA.getId(), agreement.getId()))
                        .header(HttpHeaders.AUTHORIZATION, authHeader)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"coefficientIds\": []}"))
                .andDo(print())
                .andExpect(status().isBadRequest());
    }

    @Test
    void reopensSelfAuthoredCloseAndIsIdempotentOnResend() throws Exception {
        setUpBaseFixture();
        SharingAgreementEntity agreement = persistAgreement(plantA, SharingAgreementStatus.PUBLISHED);
        SupplyPartitionCoefficientEntity closed = persistCoefficient(supplyA, plantA, agreement,
                Instant.parse("2024-01-01T00:00:00Z"), Instant.parse("2024-06-01T00:00:00Z"));
        String authHeader = loginAsCommunityAdmin(communityA.getId());
        String requestBody = body(closed.getId());
