package org.lucoenergia.conluz.infrastructure.production.sharingagreement.deactivate;
...
@Transactional
class DeactivateCoefficientsControllerTest extends BaseControllerTest {
    ...
    private String url(UUID plantId, UUID agreementId) {
        return "/api/v1/plants/" + plantId + "/sharing-agreements/" + agreementId + "/partition-coefficients/deactivate";
    }
    ...
    @Test
    void revertsActiveCoefficientToPendingAndIsIdempotentOnResend() throws Exception {
        setUpBaseFixture();
        SharingAgreementEntity agreement = persistAgreement(plantA, SharingAgreementStatus.PUBLISHED);
        SupplyPartitionCoefficientEntity coefficient = persistCoefficient(supplyA, plantA, agreement,
                Instant.parse("2024-01-01T00:00:00Z"));
        String authHeader = loginAsCommunityAdmin(communityA.getId());
        String requestBody = body(coefficient.getId());
