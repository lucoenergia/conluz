...
This policy is MANDATORY. Every REST controller endpoint MUST enforce it via a `@PreAuthorize` clause (and, where object-level scope is required, via the `@communityAccessGuard` bean or an equivalent service-layer check). The `@Operation` description MUST state the required role(s) so Swagger matches the guard. No endpoint may rely on being "internal" — every endpoint is authorized.
...
- Object reads scoped to ownership/community: enforce `canReadSupply` / `canReadCommunity`, in the controller `@PreAuthorize` OR the service before returning data. List endpoints MUST filter results by `visibleCommunityIds()`.
- `isAuthenticated()` alone is acceptable ONLY when the service performs the object-level scope check (e.g. `GetSupplyServiceImpl.getById`, `GetDatadisConsumptionServiceImpl`). Document that in the method.
- Self-service: a user editing their own record is allowed; enabling/disabling/deleting one's own account MUST be rejected for everyone, including admins.
